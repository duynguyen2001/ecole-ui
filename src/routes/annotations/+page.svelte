<script lang="ts">
	...
	let snapshots: ImageData[][] = []; // history of all the snapshots, updated when there is a change in any image
	...
	const MAX_HEIGHT_LARGE = 500;
	const MAX_WIDTH_LARGE = 500;
	let doneLoading = false;

	// handle hover action on a polygon to show the label
	let showLabel = false;
	let labelText = "";
	let labelX = 0;
	let labelY = 0;
	...
	const convertOriginalPointsToStageCoordinates = (
		...
		const newPoints = points.map((point, i) => {
			if (i % 2 === 0) return (maxWidth - image_info?.scaledWidth) / 2 + point * image_info.scale;
			else return (maxHeight - image_info?.scaledHeight) / 2 + point * image_info.scale;
		});
		return newPoints;
	};
	...
	const convertStageCoordinatePointsToOriginal = (
		...
		if (!image_info) return [];
		const newPoints = points.map((point, i) => {
			if (i % 2 === 0) return (point - (maxWidth - image_info?.scaledWidth) / 2) / image_info.scale;
			else return (point - (maxHeight - image_info?.scaledHeight) / 2) / image_info.scale;
		});
		return newPoints;
	};
		...
		const handleKeyDown = (event: KeyboardEvent) => {
			if ((event.ctrlKey || event.metaKey) && event.key === "z") {
				...
				if (snapshots.length > 1 && currentImageIndex !== -1) {
					console.log(
						snapshots[snapshots.length - 1][currentImageIndex].origin.polygonPoints[1].points
					);
					snapshots.pop();
	...
	function actionSaveAnnotationChanges(
		...
		originalPoints[0] = Math.round(originalPoints[0] * 100) / 100;
		originalPoints[1] = Math.round(originalPoints[1] * 100) / 100;
		...
		console.log(originalPoints[0], originalPoints[1]);
		...
		// console.log(images[currentImageId].origin.polygonPoints[polygonIndex].points);
		// console.log(newSnapshot[currentImageId].origin.polygonPoints[polygonIndex].points);
...
<div class="flex-start flex h-full flex-col gap-[5px]">
	...
	<div class="flex w-full grow flex-row gap-[30px]">
													...
													<Line
														config={{
															...
															fill: addTransparency(
																originLabelsInfo?.get(image_info.origin.polygonPoints[i0].label)
																	?.color,
																0.5
															),
															hitStrokeWidth: 0,
		...
		<div class="h-full w-1/2 border-[1px] border-black">
			{#if currentImage}
				<div class="flex items-center justify-center" role="button" tabindex="-1">
					<Stage class="inline-block" config={{ width: MAX_WIDTH_LARGE, height: MAX_HEIGHT_LARGE }}>
								...
								{#each currentImage.convertedPolygons as polygon, i0}
									...
									>
										<Line
											config={{
												points: polygon,
												stroke: labelsInfo?.get(currentImage.origin.polygonPoints[i0].label)?.color,
												strokeWidth: 2,
												closed: true,
												lineJoint: "round",
												fill: addTransparency(
												...
												>
													<Circle
														config={{
															x: polygon[i1],
															y: polygon[i1 + 1],
															fill: labelsInfo?.get(currentImage.origin.polygonPoints[i0].label)
																?.color,
															radius: 4,
															stroke: "black",
															strokeWidth: 1,
						...
						{/if}
						{#if showLabel}
							<div class="absolute" style="top: {labelY}px; left: {labelX}px;">{labelText}</div>
						{/if}
					</Stage>
				</div>
			{/if}
		</div>
		<div class="w-1/4 border-[1px] border-black">
